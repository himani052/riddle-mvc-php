...
ALTER TABLE `COURSE`
    ADD CONSTRAINT fk_course_has_creator FOREIGN KEY (creatorCourse) REFERENCES `USER` (emailUser) ON DELETE CASCADE ON UPDATE CASCADE;


ALTER TABLE `SCORE_USER_COURSE`
    ADD CONSTRAINT fk_user_has_courses_score FOREIGN KEY (user_emailUser) REFERENCES `USER` (emailUser) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE `SCORE_USER_COURSE`
    ADD CONSTRAINT fk_course_score_has_users FOREIGN KEY (course_idCourse) REFERENCES `COURSE` (idCourse) ON DELETE CASCADE ON UPDATE CASCADE;


ALTER TABLE `COMMENT`
    ADD CONSTRAINT fk_user_has_courses_comments FOREIGN KEY (user_emailUser) REFERENCES `USER` (emailUser) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE `COMMENT`
    ADD CONSTRAINT fk_course_comment_has_users FOREIGN KEY (course_idCourse) REFERENCES `COURSE` (idCourse) ON DELETE CASCADE ON UPDATE CASCADE;


ALTER TABLE `LOCATION`
    ADD CONSTRAINT fk_course FOREIGN KEY (course_idCourse) REFERENCES `COURSE` (idCourse) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE `LOCATION`
    ADD CONSTRAINT fk_department FOREIGN KEY (department_codeDepartment) REFERENCES `DEPARTMENT` (codeDepartment) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE `LOCATION`
    ADD CONSTRAINT fk_city FOREIGN KEY (city_codeCity) REFERENCES `CITY` (codeCity) ON DELETE CASCADE ON UPDATE CASCADE;


ALTER TABLE `RIDDLE`
    ADD CONSTRAINT fk_location FOREIGN KEY (location_idLocation) REFERENCES `LOCATION` (idLocation) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `CLUE`
    ADD CONSTRAINT fk_riddle FOREIGN KEY (riddle_idRiddle) REFERENCES `RIDDLE` (idRiddle) ON DELETE CASCADE ON UPDATE CASCADE;
...
-- Création des villes
INSERT INTO `CITY` (`codeCity`, `titleCity`)
...
INSERT INTO `COURSE` (`idCourse`, `titleCourse`, `descriptionCourse`, `imageCourse`, `creatorCourse`)
VALUES (1, 'Le monument oublié',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Morbi iaculis tellus sed libero iaculis, a dignissim lacus blandit. Praesent lacinia ligula dui',
        '/public/assets/img/jpg/courses/image-1.jpg', 'houssam.imani@gmail.com'),
       (2, 'En route vers le lac salé',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Morbi iaculis tellus sed libero iaculis, a dignissim lacus blandit. Praesent lacinia ligula dui',
        '/public/assets/img/jpg/courses/image-2.jpg', 'houssam.imani@gmail.com'),
       (3, 'Fin des cours !',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Morbi iaculis tellus sed libero iaculis, a dignissim lacus blandit. Praesent lacinia ligula dui',
        '/public/assets/img/jpg/courses/image-3.jpg', 'hissani.imani@gmail.com'),
       (4, 'la forêt enchanté',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Morbi iaculis tellus sed libero iaculis, a dignissim lacus blandit. Praesent lacinia ligula dui',
        '/public/assets/img/jpg/courses/image-4.jpg', 'hissani.imani@gmail.com'),
       (5, 'Afterwork mouvementé',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Maecenas a laoreet dolor. Phasellus ullamcorper neque eget ante dapibus varius sed in lectus. Praesent et arcu et lorem convallis aliquam ac et dui. Vestibulum ullamcorper velit lectus,\r\n<br><br> et luctus massa vulputate sed. Quisque dignissim metus nisl, non finibus felis feugiat nec. In rhoncus ante ac urna venenatis sodales. Curabitur velit magna, facilisis sit amet mattis in, blandit non urna. Sed ac risus ac tortor semper malesuada non in augue. Fusce luctus lobortis ipsum ac blandit. Vivamus nunc ex, auctor vitae nunc ac, feugiat tincidunt arcu. Nullam iaculis tellus eget enim laoreet, sagittis aliquam risus blandit.<br><br>Aliquam elementum mauris vel lorem iaculis, quis ultricies augue facilisis. Integer egestas suscipit leo, molestie elementum nisl feugiat at. Proin sagittis varius massa ut malesuada. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas.\r\n',
        '/public/assets/img/jpg/courses/image-5.jpg', 'tom.orhon@gmail.com'),
       (6, 'La bibliothèque ensorcelée ',
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Morbi iaculis tellus sed libero iaculis, a dignissim lacus blandit. Praesent lacinia ligula dui',
        '/public/assets/img/jpg/courses/image-6.jpg', 'tom.orhon@gmail.com');



INSERT INTO `LOCATION` (`idLocation`, `titleLocation`, `descriptionLocation`, imageLocation, addressLocation,
                        department_codeDepartment, city_codeCity, course_idCourse)
VALUES (1, 'Batîment V1 - Université de Toulon', 'Derrière le bâtiment EVE', 'location-1-course-1.jpg',
        'Av. de l Université', 83, 83000, 1),
       (2, 'Bibliothèque de Toulon - Université de Toulon', 'Face à la fac de biologie', 'location-2-course-1.jpg',
        'Av. de l Université', 83, 83000, 1),
       (3, 'Fac de droit - Universitaire Aix-Marseille', 'Devant arrêt du car 72', 'location-1-course-3.jpg',
        'Jardin du Pharo, 58 Boulevard Charles Livon', 13, 13400, 1),
       (4, 'Fac de biologie - Universitaire Aix-Marseille', 'Campus de Lumini', 'location-1-course-4.jpg',
        '163 Av, de Luminy', 13, 13000, 1);
...
VALUES (1, 'bâtiment', 'Combien de batiments compte l université de Toulon ? ', NULL, '15', 1),
       (2, 'Object disparue',
        'Retrouve dans la bibliothèques l objets manquants. Dès que tu aura retrouvé indique les 3 premières lettres dans le champ texte',
        NULL, 'liv', 2),
       (3, 'Retrouve un livre',
        'Retrouve le livre des mysérables de Victor Hugo et va à la page 100, ligne 6, caractère 10. Note la lettre',
        NULL, 'a', 2),
